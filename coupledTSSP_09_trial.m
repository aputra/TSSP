...
delta_t = 0.1;          % time step interval
...
psi_1 = psi_cc/sqrt(norm_fac);%*sqrt(10/11);
psi_2 = psi_cc/sqrt(norm_fac);%*sqrt(1/11);

tTotal = round(1500/delta_t);
N1 = zeros(1,tTotal);
N2 = zeros(1,tTotal);
...
for tt = 1:tTotal
    ...
    psi_1 = psi_1/sqrt(N1(tt));%*sqrt(10/11);
    psi_2 = psi_2/sqrt(N2(tt));%*sqrt(1/11);
    N1(tt) = sum(sum(abs(psi_1).^2)*h)*h;
    N2(tt) = sum(sum(abs(psi_2).^2)*h)*h;
