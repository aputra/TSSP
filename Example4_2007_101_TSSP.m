...
delta_t = 0.001;          % time step interval

h = 1/8;           % h is the mesh size: h = (bx-ax)/Mx = (dy-cy)/Ny
ax = -12+h; bx = 12-h;    % xmin and xmax
cy = -12+h; dy = 12-h;    % ymin and ymax
...
% %     V2D_2 = 1/2*(xx.^2 + yy.^2) - 10 - 0.499*(ff*cos(20.54*(tt-1)*delta_t) + gg*sin(20.54*(tt-1)*delta_t)) ; % xj * yk
    ...
    figure(1); subplot(1,2,2); surf(y_k,x_j,abs(psi_2).^2); shading interp; view(0,90); title(tt*delta_t);
end
