...
for tt = 1:tTotal
    ...
    [gy,gx] = gradient(psi_1,h);
    mu_chem(tt) = sum(sum(1/2*(abs(gx).^2+abs(gy).^2) +V2D_1.*abs(psi_1).^2 + beta2D.*abs(psi_1).^4 + Omega*conj(psi_1).*(1i*(xx.*gy-yy.*gx)) )) *h*h;
    mu_chem(tt) = mu_chem(tt)/sqrt(N1(tt));
...
%     figure(1); hold on; plot(tt*delta_t, step_diff, 'bx');
    ...
    figure(1); subplot(1,2,2); hold on; plot(tt*delta_t,Energy(tt),'bx');
end
